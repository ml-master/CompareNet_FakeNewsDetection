...
class DataLoader:
    ...
    def load_adj_and_other(self, path):
        ...
        if path[-1] == '/':
            ...
            # files = files[: DEBUG_NUM] if self.params.DEBUG else files[:15729]
            files = files[: DEBUG_NUM] if self.params.DEBUG else files[:15729]
    ...
    def read_dataset(self, filename, w2i, adj_file):
        ...
        if 'json' in filename:
            return self.read_dataset_sentence_json(filename, w2i, adj)
        ...
        with open(filename, "r") as f:
            ...
            for idx, (tag, doc) in tqdm(enumerate(readCSV)):
                ...
                if tag in allowed_tags:
                    ...
                    if len(sentences_idx) > 1 and len(sentences_idx) < 1000 and adj[idx][0].shape[0] == len((sentences_idx[:self.params.max_sents_in_a_doc], tag)[0]):
                        data.append((sentences_idx[:self.params.max_sents_in_a_doc], tag))
                        new_adj.append(adj[idx])
                        new_fea.append(fea[idx])
                    # if adj[idx][0].shape[0] != len(data[idx-len(removed_idx)][0]):
                    #     print('not match')
                    #     print(idx)
                    else:
                        removed_idx.append(idx)
    ...
    def read_dataset_sentence_json(self, filename, w2i, adj):
        data, new_adj, new_fea = [], [], []
        # count = 0
        adj, fea = zip(*adj)
        with open('./data/gossipcop_v3_origin.json', 'r', encoding='utf-8') as file:
            ...
            removed_idx = []
            for idx, item in tqdm(enumerate(dict_list)):
                if self.params.DEBUG and idx >= DEBUG_NUM:
                    break
                tag = item['tag']
                doc = item['text']
                sentences = sentence_tokenize(doc)
                tag = int(tag)
                allowed_tags = [2, 4] if self.ntags == 2 else [1, 2, 3, 4]
                if tag in allowed_tags:
                    if self.ntags == 2:
                        tag = tag - 2 if tag == 1 else tag - 3  # Adjust the tag to {0: Hoax, 1: Trusted}
                    else:
                        tag -= 1  # {0: Satire, 1: Hoax, 2: Propaganda, 3: Trusted}
                sentences_idx = []
                for sentence in sentences:
                    ...
                    curr_sentence_idx = [w2i[x] for x in sentence]
                    sentences_idx.append(curr_sentence_idx if len(curr_sentence_idx) > 0 else [w2i['<unk>']])
                if len(sentences_idx) > 1 and len(sentences_idx) < 1000 and adj[idx][0].shape[0] <= len(sentences_idx):
                    data.append((sentences_idx[:adj[idx][0].shape[0]], tag))
                    new_adj.append(adj[idx])
                    new_fea.append(fea[idx])
                # if adj[idx][0].shape[0] != len(data[idx-len(removed_idx)][0]):
                #     print('not match')
